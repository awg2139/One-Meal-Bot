...
if not st.session_state.hide_content:
    ...
    with col3:
        if st.button("BMR"):
            st.session_state.show_bmr = not st.session_state.show_bmr
...
# BMR
if "show_bmr" not in st.session_state:
    st.session_state.show_bmr = False
    
if st.session_state.show_bmr:
    sex = st.radio("성별",["남성", "여성"])
    age = st.number_input("나이",min_value=1)
    height = st.number_input("키 (cm)", min_value=50)
    weight = st.number_input("몸무게 (kg)", min_value=20)
